...
import { CalendarDays, FileText, GraduationCap, Plus, Users } from "lucide-react";
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            {getGreeting(firstName)} · Welcome to {institution.shortName} Dashboard
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <ColorStatCard
          ...
          value={formatNumber(institution.studentCount)}
          change="+0.5%"
          changeType="up"
          icon={Users}
          tone="purple"
        ...
        <ColorStatCard
          ...
          value={formatNumber(institution.staffCount)}
          change="-3%"
          changeType="down"
          icon={GraduationCap}
          tone="coral"
        ...
        <ColorStatCard
          label="Events"
          value={String(Math.max(events.length, upcomingEvents.length))}
          change="+6%"
          changeType="up"
          icon={CalendarDays}
          tone="teal"
        ...
        >
          <div className="mb-2 flex gap-6">
            <div>
              ...
              <p className="text-lg font-bold text-[#1BD0B4]">{formatCurrency(469244)}</p>
            </div>
            <div>
              <p className="text-xs text-[var(--muted)]">Expense</p>
              <p className="text-lg font-bold text-[#F4901F]">{formatCurrency(33456)}</p>
            </div>
          </div>
